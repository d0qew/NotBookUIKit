...
class SecondMounthTableViewController: UITableViewController {
    ...
    @IBAction func pushNewGoal(_ sender: Any) {
        ...
        alertController.addTextField{(textField) in
            textField.placeholder = "New mounthly goal"
        }
        ...
        let alertActionSecond =  UIAlertAction(title: "Add", style: .cancel) { (alert) in
            // create new
        let newItemMounthly = alertController.textFields![0].text
        addNewMounthlyGoals(nameGoalsItem: newItemMounthly!)
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return mounthlyGoals.count
    }

    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Identifier", for: indexPath)
        let current = mounthlyGoals[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            removeMounthlyGoals(at: indexPath.row)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if changeStateMounthGoal(at: indexPath.row){
